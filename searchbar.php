...
if(!isset($page))
{
	    		$url = 'http://pedrocosta.heliohost.org/multimedia/search.php?page=1&search='.$search;
echo'
<script>
...
$aux=($page - 1) * 6 ;
$raux=mysqli_query($conn,"SELECT id FROM livros WHERE name LIKE '%{$search}%' ");
$limit=6;
...
if(isset($_POST['nextpage']))
{
	if($page==$lastpage)
	{
	$url = 'http://pedrocosta.heliohost.org/multimedia/search.php?page='.$page.'&search='.$search;
echo'
<script>
window.location.href = "'.$url.'";
</script>
...
<body>
	...
	<nav>
		...
		<ul class="menu">
			...
			<li>
				<form method="GET">
					<input name="search"type="text" style="margin-top: 20px; text-align: center" class="pure-input-rounded" placeholder=<?php echo $search;?> >
	...
	<form method="POST">
		<?php
		$stuff=mysqli_query($conn,"SELECT name,autor,imagem,link FROM livros WHERE name LIKE '%{$search}%' LIMIT ".$aux." , 6");
