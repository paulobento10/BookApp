...
<body>
		...
		<ul class="menu">
			...
			<li>
				<form method="GET" action="searchbar.php">
					...
					<button name="sub" type="submit" href="searchbar.php?search="  class="pure-button pure-button-primary">Search</button>
					<?php
					if(isset($_GET['sub']))
					{
						header('location: searchbar.php?page=1&search='.$_GET['search']);
					}
					?>
	...
	<form method="POST">
		<?php
		$stuff=mysqli_query($conn,"SELECT name,autor,imagem,link FROM livros ORDER BY classificacao DESC LIMIT 10");
