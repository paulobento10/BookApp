...
if(!isset($_GET['page']))
{
$url = 'http://pedrocosta.heliohost.org/multimedia/books4kids.php?page=1';
echo'
<script>
...
$aux=($page - 1) * 6 ;
$raux=mysqli_query($conn,"SELECT id FROM livros");
$limit=6;
...
{
$url = 'http://pedrocosta.heliohost.org/multimedia/books4kids.php?page='.$page;
echo'
<script>
window.location.href = "'.$url.'";
</script>
...
<body>
		...
		<ul class="menu">
			...
			<li>
				<form method="GET" action="searchbar.php" >
					<input name="search" type="text" style="margin-top: 20px; text-align: center" name="search" class="pure-input-rounded" placeholder="insira um titulo" >
					...
					if(isset($_GET['sub']))
					{
  						...
  						header('location: searchbar.php?page=1&search='.$search);
					}
					?>
	...
	<form method="POST">
		<?php
		$stuff=mysqli_query($conn,"SELECT name,autor,imagem,link FROM livros LIMIT ".$aux." , 6");
		...
		<div method="POST" action="books4kids.php" class="pure-button-group pure-u-1 form-box"  style="margin-top: 100px;border: 0">
			<?php if($page>1)
			{?>
				...
				<form class="pure-u-1 form-box" style="margin-top: 20px" method="GET">
					...
					<form class="pure-form" method="GET">
						...
						if(isset($_GET['send']))
						{
							...
							header('location: books4kids.php?page='.$page);
						}
						?>
					</form>
				</div>
